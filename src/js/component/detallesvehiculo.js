...
export const VehiculoDetails = () => {
    ...
    useEffect(() => {
        ...
        const getImage = async () => {
            try {
                const response = await fetch(`https://starwars-visualguide.com/assets/img/vehicles/${id}.jpg`);
                if (response.ok) {
                    setImage(`https://starwars-visualguide.com/assets/img/vehicles/${id}.jpg`);
                } else {
                    console.error(`No se pudo obtener la imagen para el vehiculo con ID ${id}`);
                }
            } catch (error) {
